...
from zunel.config.loader import (  # noqa: E402
    _resolve_env_vars,
    load_config,
...
class TestResolveEnvVars:
    ...
    def test_partial_replacement(self, monkeypatch):
        ...
        assert _resolve_env_vars("https://${HOST}/api") == (
...
class TestResolveConfig:
    def test_resolves_env_vars_in_config(self, tmp_path, monkeypatch):
        ...
        config_path.write_text(
            json.dumps(
                {"providers": {"custom": {"apiKey": "${TEST_API_KEY}"}}}
            ),
            encoding="utf-8",
        )

        raw = load_config(config_path)
        assert raw.providers.custom.api_key == "${TEST_API_KEY}"

        resolved = resolve_config_env_vars(raw)
        assert resolved.providers.custom.api_key == "resolved-key"
    ...
    def test_preserves_excluded_fields_with_env_refs(
