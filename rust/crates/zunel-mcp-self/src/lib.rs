...
//!
//! Originally `zunel-mcp-self` was a binary-only crate that hard-wired
//! the self-inspection tool set into the JSON-RPC dispatcher. To let
//! other commands (notably `zunel mcp agent --profile NAME`) reuse the
//! same Streamable-HTTP/HTTPS transport with a *different* tool set,
//! the dispatcher is now expressed behind the [`McpDispatcher`] trait
//! and the transport layer in [`http`] runs against any implementation
//! of that trait.
//!
//! Two dispatchers ship in this crate:
//!
//! - [`SelfDispatcher`] — the historical zunel-self tools (sessions,
//!   cron, channels, token usage, …). Backs both the stdio binary
//!   entry point and `zunel mcp serve`.
...
pub struct DispatchMeta {
    ...
    /// session ids; v1 dispatchers ignore it.
