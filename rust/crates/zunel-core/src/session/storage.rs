...
//!
//! Mirrors Python's `zunel/session/manager.py::SessionManager` byte-for-byte
//! so a Python-written JSONL file round-trips through Rust unchanged. The
//! [`PythonCompactFormatter`] reproduces `json.dumps(ensure_ascii=False)`'s
//! default `(", ", ": ")` separators, and atomic save uses the
//! tempfile-then-rename trick so concurrent loaders never see a partial
//! write.
...
impl SessionManager {
    ...
    pub fn save(&self, session: &Session) -> crate::Result<()> {
        ...
        // Python's spacing via `PythonCompactFormatter`.
