...
mod tests {
    ...
    /// Regression: before the `truncate_at_char_boundary` migration,
    /// `render_transcript` used `&content[..1500]` which panicked
    /// with `byte index 1500 is not a char boundary; it is inside '─'
    /// (bytes 1499..1502)` whenever a session message contained a
    /// terminal banner with U+2500 box-drawing characters around the
    /// 1500-byte cut. That panic killed the in-flight Slack worker —
    /// users saw the eyes reaction but no reply.
    #[test]
