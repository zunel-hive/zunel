...
//!
//! Streaming, approval-forwarding, and per-call timeouts are explicit
//! v2 non-goals — see [`docs/profile-as-mcp-mode2.md`](../../../../../../../docs/profile-as-mcp-mode2.md).
//!
//! ## Threading model
//!
    ...
    async fn execute(&self, args: Value, ctx: &ToolContext) -> ToolResult {
        ...
        let agent =
            AgentLoop::with_sessions(self.provider.clone(), defaults, (*self.sessions).clone())
                ...
                // flag's value is moot — we still set it true for
                // consistency with the docs.
                .with_approval_required(true)
                .with_approval_scope(ApprovalScope::All);

        // Drain the streaming sink locally; slice 1 is single-JSON
        // only, so we don't forward deltas anywhere. The drain task
        // is aborted right after the call completes.
...
mod tests {
    ...
    async fn helper_ask_returns_text_and_meta_with_namespaced_session() {
        ...
        // Empty tools_used is the right answer for slice 1: the
        // helper's loop didn't call any tools (we registered an
        // empty inner registry).
