...
pub(super) async fn run(args: McpAgentArgs, config_path: Option<&Path>) -> Result<()> {
    ...
    if args.mode2 {
        ...
        let provider =
            zunel_providers::build_provider(&cfg).context("building provider for Mode 2")?;
...
/// `mcp_*` tools are always pruned in v1: the depth-forwarding plumbing
/// is now in place, but the design doc defers MCP-of-MCP re-export
/// pending product decisions about per-call timeouts, OAuth-token
/// visibility across hops, and audit logging.
