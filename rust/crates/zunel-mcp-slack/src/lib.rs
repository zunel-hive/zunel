...
/// surface — this matches the historical behavior of the standalone
/// `zunel-mcp-slack` binary and keeps the unit tests deterministic. Hosts
/// that want to enforce read-only or allowlist-scoped writes must set the
/// flags explicitly in `~/.zunel/config.json`.
