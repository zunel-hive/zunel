...
| Profile-as-MCP | [`profile-as-mcp.md`](./profile-as-mcp.md) | Expose a named profile's tool registry as a Streamable HTTP/HTTPS MCP server (`zunel mcp agent`) |
| Profile-as-MCP Mode 2 | [`profile-as-mcp-mode2.md`](./profile-as-mcp-mode2.md) | Draft design for `helper_ask` — running a helper profile's full agent loop as a single MCP tool. Not yet implemented. |
