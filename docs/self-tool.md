...
The Rust `self` tool lets the agent inspect safe read-only runtime state in
local CLI sessions, the Slack-backed gateway, and the Rust library facade.
...
`self` is registered automatically by the Rust runtime. There is no `tools.my`
